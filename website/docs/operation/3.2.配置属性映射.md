...
通过在目标对象的字段上添加`@Mapping`注解，并设置对应的`src`和`ref`属性，我们告知装配操作处理器哪些属性需要进行映射以及属性之间的对应关系。

<img src="https://img.xiajibagao.top/image-20230220182129822.png" alt="image-20230220182129822" style="zoom: 50%;" />

## 3.2.1.将属性映射到属性

字段映射配置类似于MapStruct，在使用`@Assemble`注解声明装配操作时，我们可以在`props`中配置字段映射，指定源属性和目标属性之间的映射关系。

在`props`中，我们可以使用`@Mapping`注解配置映射属性。例如：
...
当我们指定了一对多或多对多的装配操作处理器时，一个键值将对应一批数据源对象。在这种情况下，`@Mapping` 注解配置的字段映射模式将变为**批量映射**

具体而言，对于集合中的每个数据源对象，我们会从中获取指定的 `src` 属性值，并将所有属性值组装为集合，然后将该集合赋值给目标对象中的 `ref` 属性。
